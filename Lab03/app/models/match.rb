class Match < ActiveRecord::Base
  ...
  def winner_looser_draw
  	if local_goals == visit_goals && (!draw? || winner_team != nil || looser_team != nil)
  		errors.add(:visit_team, "ERROR: Local team and visit team scored the same quantity of goals, they must draw, winner and looser team must be nil")
  	elsif local_goals > visit_goals && (draw? || winner_team != local_team || looser_team != visit_team)
  		errors.add(:visit_team, "ERROR: Local team scored more goals than visit team, local team must win and visit team must loose, no draw allowed")
  	elsif local_goals < visit_goals && (draw? || winner_team != visit_team || looser_team != local_team)
      errors.add(:visit_team, "ERROR: Visit team scored more goals than local team, visit team must win and local team must loose, no draw allowed")
  	end
  end
